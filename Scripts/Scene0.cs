...
public class testing : MonoBehaviour {
    ...
    public void updateusersAgeYrs() {
        ...
        if (isLeapYear(currentYear)) {
            curDate_daysPerMonth[1] = 29;
        }
        int curDate_daysFromMonths = 0;
        for (int i = 0; i <= curMonth - 2; i++) {
            curDate_daysFromMonths += curDate_daysPerMonth[i];
        }
        List<int> birthday_daysPerMonth = new List<int>{31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31};
        if (isLeapYear(currentlySelectedYear())) {
            birthday_daysPerMonth[1] = 29;
        }
        int birthday_daysFromMonths = 0;
        for (int i = 0; i <= currentlySelectedMonth() - 2; i++) {
            birthday_daysFromMonths += birthday_daysPerMonth[i];
        }
        ...
        for (int i = currentlySelectedYear(); i < currentYear; i++) {
            if (isLeapYear(i)) {
                daysPassedFromYears += 366;
            }
            else {
                daysPassedFromYears += 365;
            }
        }
    ...
    int currentlySelectedYear() {
        return currentYear - YearOptions.GetComponent<TMPro.TMP_Dropdown>().value;
    }
    int currentlySelectedDay() {
        return DayOptions.GetComponent<TMPro.TMP_Dropdown>().value + 1;
    }
    int currentlySelectedMonth() {
        return MonthOptions.GetComponent<TMPro.TMP_Dropdown>().value + 1;
    }
    int currentlySelectedFallingAsleepDuration() {
        return 10 + FallAsleepDuration.GetComponent<TMPro.TMP_Dropdown>().value * 5;
    }
    int currentlySelectedWakeUpTime() {
        return WakeUpTimeOptions.GetComponent<TMPro.TMP_Dropdown>().value * 5;
    }
    ...
    public void userEditsBirthdayMonth() {
        ...
        if (daysPerMonth[usersBirthdayMonth - 1] != daysPerMonth[currentlySelectedMonth() - 1]) {
            ...
            for (int i = 1; i <= daysPerMonth[currentlySelectedMonth() - 1]; i++) {
                dayOptions.Add(i.ToString());
            }
            DayOptions.GetComponent<TMPro.TMP_Dropdown>().AddOptions(dayOptions);
            if (usersBirthdayDay <= dayOptions.Count) {
    ...
    public void goToExplanationPage() {
    ...
    string timeInMinToString(int timeInMin) {
        ...
        string timeAsString;
        if (timeInMin / 60 < 1) {
            timeAsString = "12:" + (timeInMin % 60).ToString("00") + "am";
        }
        else if (timeInMin / 60 >= 12 && timeInMin / 60 < 13) {
            timeAsString = "12:" + (timeInMin % 60).ToString("00") + "pm";
        }
        else if (timeInMin / 60 >= 13) {
            timeAsString = (timeInMin / 60 - 12).ToString() + ":" + (timeInMin % 60).ToString("00") + "pm";
        }
        else {
            timeAsString = (timeInMin / 60).ToString() + ":" + (timeInMin % 60).ToString("00") + "am";
        }
        return timeAsString;
    }
    ...
    int sleepNeededMinutes() {
        ...
        if (usersAgeYrs >= 0.00000f  && usersAgeYrs < 0.66667f) {
            sleepNeededHrs = (-4.00000f * usersAgeYrs) + 16.16667f;
        }
        else if (usersAgeYrs >= 0.66667f  && usersAgeYrs < 2.00000f) {
            sleepNeededHrs = (-0.75000f * usersAgeYrs) + 14.00000f;
        }
        else if (usersAgeYrs >= 2.00000f  && usersAgeYrs < 4.50000f) {
            sleepNeededHrs = (-0.40000f * usersAgeYrs) + 13.33333f;
        }
        else if (usersAgeYrs >= 4.50000f  && usersAgeYrs < 10.00000f) {
            sleepNeededHrs = (-0.27273f * usersAgeYrs) + 12.72727f;
        }
        else if (usersAgeYrs >= 10.00000f && usersAgeYrs < 16.00000f) {
            sleepNeededHrs = (-0.16667f * usersAgeYrs) + 11.66667f;
        }
        else if (usersAgeYrs >= 16.00000f && usersAgeYrs < 22.00000f) {
            sleepNeededHrs = (-0.16667f * usersAgeYrs) + 11.66667f;
        }
        else if (usersAgeYrs >= 22.00000f && usersAgeYrs < 45.50000f) {
            sleepNeededHrs = (-0.00000f * usersAgeYrs) + 8.00000f;
        }
        else if (usersAgeYrs >= 45.50000f && usersAgeYrs < 122.44901f) {
            sleepNeededHrs = (-0.01923f * usersAgeYrs) + 8.87500f;
        }
    ...
    public void userEditsBirthdayYear() {
        ...
        if (currentlySelectedMonth() == 2 && isLeapYear(usersBirthdayYear) != isLeapYear(currentlySelectedYear())) {
            ...
            for (int i = 1; i <= numDaysFeb; i++) {
                dayOptions.Add(i.ToString());
            }
            DayOptions.GetComponent<TMPro.TMP_Dropdown>().AddOptions(dayOptions);
            if (usersBirthdayDay <= numDaysFeb) {
                DayOptions.GetComponent<TMPro.TMP_Dropdown>().value = usersBirthdayDay - 1;
            }
    ...
    void Start() {
        ...
        for (int i = currentYear; i >= currentYear - 122; i--) {
        ...
        for (int i = 10; i <= 20; i += 5) {
            fallingAsleepDurationOptions.Add(i.ToString() + " min");
        }
        ...
        for (int i = 0; i <= 55; i += 5) {
            wakeUpTimeOptions.Add("12:" + i.ToString("00") + "am");
        }
        for (int i = 1; i <= 11; i++) {
            for (int j = 0; j <= 55; j += 5) {
                wakeUpTimeOptions.Add(i.ToString() + ":" + j.ToString("00") + "am");
            }
        }
        for (int i = 0; i <= 55; i += 5) {
            wakeUpTimeOptions.Add("12:" + i.ToString("00") + "pm");
        }
        for (int i = 1; i <= 11; i++) {
            for (int j = 0; j <= 55; j += 5) {
                wakeUpTimeOptions.Add(i.ToString() + ":" + j.ToString("00") + "pm");
            }
        }
